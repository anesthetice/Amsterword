...
fn main() -> io::Result<()> {
    ...
    let generic_word: String = config
        .lexicon
        .choose(&mut rng)
        .ok_or(lexicon_choose_error())?
        .clone();
    let anomaly_word: String = {
        let mut word: String = config
            .lexicon
            .choose(&mut rng)
            .ok_or(lexicon_choose_error())?
            .clone();
    ...
    list_of_words.push(anomaly_word);
    ...
    for num in 0..config.number_of_players {
        ...
        if selected_word == generic_word {
            ...
            println!(
                "-------------- PLAYER {} --------------\n\nyour word is : '{}'\n",
                num,
                if config.hint_system {
                    if rng.gen_bool(config.hint_probability_generic) {
                        selected_word.red().bold()
                    } else {
                        selected_word.green().bold()
                    }
                } else {
                    selected_word.cyan().bold()
                }
            );
            pause()?;
            sleep(0.2);
            ...
            println!(
                ...
                num,
                if config.hint_system {
                    if rng.gen_bool(config.hint_probability_anomaly) {
                        selected_word.red().bold()
                    } else {
                        selected_word.green().bold()
                    }
